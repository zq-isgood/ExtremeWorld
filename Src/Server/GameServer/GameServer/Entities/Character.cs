...
namespace GameServer.Entities
{
    class Character : Creature,IPostResponser
    {
        ...
        public Team Team;  //组队没有DB,所以创建一个类叫Team
        public double TeamUpdateTS;  //时间戳，默认是0，每个玩家的值都不一样
        public Guild Guild;
        public double GuildUpdateTS;
        public Chat Chat; //聊天不用存DB
        ...
        {
            this.Level += 1;
            Log.InfoFormat("Character[{0}:{1}] LevelUp:{2}", Id, this.Info.Name, this.Level);
            CheckLevelUp();
        }
        ...
        public void PostProcess(NetMessageResponse message)
        {
            ...
            if (this.Guild!= null)
            {
                ...
                {
                    this.Info.Guild = this.Guild.GuildInfo(this); //如果是公会后面创建的，则后处理时把工会的信息填一次
                    if (message.mapCharacterEnter != null)
                        GuildUpdateTS = Guild.timestamp; //如果不是第一次登录，则不设置
                }
